...
const Navbar = () => {
    ...
    return (
        ...
        <div className='flex items-center justify-between text-sm py-4 mb-5 border-b-4 border-b-black bg-white'>
            ...
            <div className='flex items-center gap-4'>
                <a href="https://github.com/laxman-thedev/Doctor-Appointment-Booking-System" class="max-md:hidden flex gap-1.5 bg-primary text-white btn py-2.5 px-5 rounded-full text-sm hover:bg-white hover:text-primary border hover:border-primary" aria-label="GitHub Repository" rel="noopener noreferrer"><svg class="size-5 shrink-0" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" viewBox="0 0 24 24"><path fill-rule="evenodd" d="M12.006 2a9.847 9.847 0 0 0-6.484 2.44 10.32 10.32 0 0 0-3.393 6.17 10.48 10.48 0 0 0 1.317 6.955 10.045 10.045 0 0 0 5.4 4.418c.504.095.683-.223.683-.494 0-.245-.01-1.052-.014-1.908-2.78.62-3.366-1.21-3.366-1.21a2.711 2.711 0 0 0-1.11-1.5c-.907-.637.07-.621.07-.621.317.044.62.163.885.346.266.183.487.426.647.71.135.253.318.476.538.655a2.079 2.079 0 0 0 2.37.196c.045-.52.27-1.006.635-1.37-2.219-.259-4.554-1.138-4.554-5.07a4.022 4.022 0 0 1 1.031-2.75 3.77 3.77 0 0 1 .096-2.713s.839-.275 2.749 1.05a9.26 9.26 0 0 1 5.004 0c1.906-1.325 2.74-1.05 2.74-1.05.37.858.406 1.828.101 2.713a4.017 4.017 0 0 1 1.029 2.75c0 3.939-2.339 4.805-4.564 5.058a2.471 2.471 0 0 1 .679 1.897c0 1.372-.012 2.477-.012 2.814 0 .272.18.592.687.492a10.05 10.05 0 0 0 5.388-4.421 10.473 10.473 0 0 0 1.313-6.948 10.32 10.32 0 0 0-3.39-6.165A9.847 9.847 0 0 0 12.007 2Z" clip-rule="evenodd"></path></svg>Star On GitHub</a>
                {
                    token && userData ? (
                        ...
                        <div className='flex items-center gap-2 cursor-pointer group relative'>
                            <img className='w-8 h-8 border-2 border-black' src={userData.image} alt="profile_pic" />
                            <img className='w-3' src={assets.dropdown_icon} alt="dropdown" />

                            {/* Dropdown menu visible on hover */}
                            <div className='absolute top-0 right-0 pt-14 text-base font-bold text-black z-20 hidden group-hover:block'>
                                <div className='min-w-48 neo-card flex flex-col p-2'>
                                    <p onClick={() => { navigate('/my-profile') }} className='hover:bg-neo-yellow px-3 py-2 border-2 border-transparent hover:border-black cursor-pointer transition-all'>My Profile</p>
                                    <p onClick={() => { navigate('/my-appointments') }} className='hover:bg-neo-cyan px-3 py-2 border-2 border-transparent hover:border-black cursor-pointer transition-all'>My Appointments</p>
                                    <p onClick={logout} className='hover:bg-neo-pink px-3 py-2 border-2 border-transparent hover:border-black cursor-pointer transition-all'>Logout</p>
                                </div>
                            </div>
                        </div>
                    ) : (
                        ...
                        <button
                            ...
                            className='neo-btn bg-neo-yellow px-8 py-3 hidden md:block'
                        >
                            Create account
                        </button>
                    )
                ...
                <div className={`${showMenu ? 'fixed w-full' : 'h-0 w-0'} md:hidden right-0 top-0 bottom-0 z-20 overflow-hidden bg-white transition-all`}>
                    ...
                    <div className='flex items-center justify-between px-5 py-6 border-b-4 border-black bg-neo-yellow'>
                    ...
                    <ul className='flex flex-col items-center gap-4 mt-8 px-5 text-lg font-bold'>
                        <NavLink onClick={() => { setShowMenu(false) }} className='w-full' to='/'><p className='w-full text-center border-2 border-black shadow-neo py-3 bg-white hover:bg-gray-100 transition-all'>HOME</p></NavLink>
                        <NavLink onClick={() => { setShowMenu(false) }} className='w-full' to='/doctors'><p className='w-full text-center border-2 border-black shadow-neo py-3 bg-neo-cyan hover:bg-cyan-200 transition-all'>ALL DOCTORS</p></NavLink>
                        <NavLink onClick={() => { setShowMenu(false) }} className='w-full' to='/about'><p className='w-full text-center border-2 border-black shadow-neo py-3 bg-neo-pink hover:bg-pink-300 transition-all'>ABOUT</p></NavLink>
                        <NavLink onClick={() => { setShowMenu(false) }} className='w-full' to='/contact'><p className='w-full text-center border-2 border-black shadow-neo py-3 bg-neo-yellow hover:bg-yellow-300 transition-all'>CONTACT</p></NavLink>
